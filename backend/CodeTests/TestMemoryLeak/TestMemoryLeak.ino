...
bool setTimer(Timers timer, int timerID){
    if (timerID >= NUMBER_OF_TIMERS || timerID < 0){ Serial.print("[Timer] over index setTimer");return 1;}
...
Timers getTimer(int timerID){
    if (timerID >= NUMBER_OF_TIMERS || timerID < 0){ Serial.print("[Timer] over index getTimer");return Timerlist[0];}
    return Timerlist[timerID];
}

bool delTimer(int timerID){
    if (timerID >= NUMBER_OF_TIMERS || timerID < 0){ Serial.print("[Timer] over index delTimer");return 1;}
...
void onMassageEvent(AsyncWebSocket *server, AsyncWebSocketClient *client, StaticJsonDocument<LAGES_SOCKET_EVENT_FOR_ARDURINO_JSON>& doc) {
    ...
    if   (!strcmp(eventtype, "setTimer")) {
        ...
        setNewTimer(timer); // ! Implement error Handeling
    ...
    if   (!strcmp(eventtype, "updateTimer")) {
        ...
        setTimer(timer_obj, doc["timerID"]); // ! Implement error Handeling
        ws.textAll(TimerRequest(doc["timerID"]));
        return;
    }
    if   (!strcmp(eventtype, "delTimer")) {
        delTimer(doc["timerID"]); // ! Implement error Handeling
